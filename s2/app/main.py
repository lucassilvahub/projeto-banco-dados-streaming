...
async def create_kafka_producer(max_retries=15, retry_delay=5):
    ...
    if producer is None:
        logger.error(
            "❌ Não foi possível iniciar o Kafka Producer após várias tentativas."
        )
        return None
...
async def create_kafka_consumer(max_retries=15, retry_delay=5):
    ...
    while consumer is None and retries < max_retries:
        try:
            ...
            if INPUT_TOPIC not in topics:
                logger.warning(
                    f"⚠️ Tópico {INPUT_TOPIC} não encontrado. Tentando criar..."
                )
            ...
            if OUTPUT_TOPIC not in topics:
                logger.warning(
                    f"⚠️ Tópico {OUTPUT_TOPIC} não encontrado. Tentando criar..."
                )
                # Tenta enviar uma mensagem para criar o tópico
                temp_producer = AIOKafkaProducer(
                    bootstrap_servers=KAFKA_BROKER,
                ...
                await temp_producer.send_and_wait(
                    ...
                    {
                        "event_type": "init",
                        "message": "Inicializando tópico de resposta",
                    },
                )
                await temp_producer.stop()
    ...
    if consumer is None:
        logger.error(
            "❌ Não foi possível iniciar o Kafka Consumer após várias tentativas."
        )
        return None
...
async def processar_evento(evento, pg_conn, mongo_db, redis_conn, producer):
    ...
    try:
        ...
        if pg_conn is not None:
            result = await postgres_handler.processar_evento_postgres(
                pg_conn, evento, producer
            )
        ...
        if mongo_db is not None:
            result = await mongodb_handler.processar_evento_mongodb(
                mongo_db, evento, producer, pg_conn
            )
            if result is not None:
                return result
...
async def iniciar_processador():
    ...
    if producer is None:
        logger.error(
            "❌ Não foi possível iniciar o produtor Kafka. Encerrando serviço."
        )
    ...
    if consumer is None:
        logger.error(
            "❌ Não foi possível iniciar o consumidor Kafka. Encerrando serviço."
        )
        await producer.stop()
        await cleanup_connections(pg_conn, mongo_client, redis_conn)
        return
    ...
    try:
        ...
        async for msg in consumer:
            evento = msg.value
            logger.info(
                f"📩 Evento recebido: {evento.get('event_type')} (correlation_id: {evento.get('correlation_id', 'N/A')})"
            )
...
async def main():
    try:
        logger.info(
            "🚀 Iniciando serviço S2 de processamento com persistência poliglota..."
        )
