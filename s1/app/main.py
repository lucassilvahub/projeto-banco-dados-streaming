...
while mongo_client is None and mongo_retry_count < max_mongo_retries:
    try:
        ...
        print(f"[S1] ⚠️ MongoDB não disponível... tentativa {mongo_retry_count+1}/{max_mongo_retries}: {e}")
        time.sleep(3)
        mongo_retry_count += 1
...
while redis_client is None and redis_retry_count < max_redis_retries:
    try:
        ...
        print(f"[S1] ⚠️ Redis não disponível... tentativa {redis_retry_count+1}/{max_redis_retries}: {e}")
        time.sleep(3)
        redis_retry_count += 1
...
def gerar_recomendacoes(user_id: int):
    ...
    for tipo in tipos_preferidos:
        ...
        quantidade = min(tipo["count"] * 2, 10)  # 2x mais recomendações do que o histórico, até 10
    ...
    if not recomendacoes:
        for _ in range(5):
            ...
            recomendacao = {
                ...
                "tipo": tipo_conteudo,
                "categoria": random.choice(categorias),
                "pontuacao": round(random.uniform(0.5, 0.8), 2),  # Score menor para recomendações aleatórias
...
def encerrar_sessao(session_id: str):
    ...
    redis_client.hset(f"session:{session_id}", "encerramento", datetime.utcnow().isoformat())
...
def obter_cache_conteudo(conteudo_id: int):
    ...
    redis_client.hset(f"conteudo:{conteudo_id}", "ultimo_acesso", datetime.utcnow().isoformat())
